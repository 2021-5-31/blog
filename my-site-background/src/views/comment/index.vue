<template>
  <div class="app-container">
    <el-table :data="data.rows" style="width: 100%" border v-loading="loading" v-if="data.rows">
      ...
      <el-table-column label="评论昵称" width="180" align="center">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.nickname }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论内容" min-width="180" align="center">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.content }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论文章" min-width="120" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    fetchData() {
      this.loading = true;
      getComments({ page: this.currentPage, limit: this.pageSize }).then(
        (res) => {
          this.data = res.data;
